...
class SavgolFilter(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y = None):
        out = X.copy()
        out[self.column+'_savgol'] = savgol_filter(out[self.column], self.window_length, self.poly_order)
...
class Derivatives(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y = None):
        ...
        for d in range(1, self.order+1):
            out[self.column+f'_div_{d}'] = np.gradient(out[self.column + ('' if d==1 else f'_div_{d-1}')])
        return out
