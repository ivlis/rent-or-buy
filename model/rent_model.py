...
from .periodic_model import GeneratePeriodic, Derivatives, SavgolFilter, SelectFeaturesR
...
class RentModel:
    ...
    def _load_features(self):
        ...
        ma_fmr = all_fmr.merge(
            ...
            left_on='fips_simple', right_on='RegionName').drop(
            columns=['fips_simple', 'RegionName'])
        ...
        for uc in range(1, 4):
            df = ma_fmr_urban_code[ma_fmr_urban_code['urban_code']==uc].set_index('Date').resample('M').interpolate(method='linear', order=2).reset_index()
        ...
        hpi_apr = loan_apr.merge(hpi[['Date', 'hpi_sa']], right_on='Date', left_index=True)
        
        smooth_hpi_apr = self._preprocess.transform(hpi_apr)  #Prepocessed features
        
        self.fmr_index = fmr_combined.groupby('Date').mean().reset_index()[['Date', 'fmr']]
    ...
    def _load_targets(self):
        ...
        for rooms in range(1, 5):
            df = house_prices[['Date', 'urban_code', f'MedianRentalPrice_{rooms}Bedroom']].copy()
            df['rooms'] = rooms
        ...
        rent_prices_combined = rent_prices_combined[
            (rent_prices_combined.Date.dt.year>2012) | (rent_prices_combined.rooms!=1) | (rent_prices_combined.urban_code!=2)
        ]
    ...
    def __init__(self):
        self._load_features()
        self._load_targets()
        self.features_and_targets = self.rent_prices_combined.merge(self.prp_features, on='Date')
